// Historical implementation retained only as a migration regression fixture. Never used by the running app.
import crypto from "crypto";
import { getPrismaAsync } from "../../db/prisma.js";
import { env } from "../../config/env.js";
import { getResumeStorage } from "../../services/resume-storage.services.js";
...
function verifyPassword(password, stored) {
  ...
  if (!Number.isSafeInteger(iterations) || iterations <= 0 || !salt || !/^[a-f0-9]{128}$/i.test(originalHash ?? "")) return false;
...
function signAccessToken(user) {
  ...
  const payload = Buffer.from(JSON.stringify({ sub: user.id, email: user.emailVerifiedAt ? user.email : undefined, name: user.name ?? null, exp })).toString("base64url");
  const signature = crypto.createHmac("sha256", "legacy-test-signing-key").update(`${header}.${payload}`).digest("base64url");
...
async function createAuthToken(prisma, userId, purpose, ttlMinutes, email) {
  ...
  await prisma.$transaction([
    ...
    prisma.authToken.create({ data: { userId, purpose, tokenHash: hashAuthToken(token), expiresAt, email } }),
  ]);
  return token;
}

async function sendVerificationEmail(user, prisma, sendEmail = sendAccountEmail) {
  const token = await createAuthToken(prisma, user.id, "EMAIL_VERIFICATION", 24 * 60, user.email);
  await deliverAuthEmail(prisma, token, sendEmail, {
    to: user.email,
...
async function deliverAuthEmail(prisma, token, sendEmail, message) {
  try {
    await sendEmail(message);
  } catch {
    await prisma.authToken.deleteMany({ where: { tokenHash: hashAuthToken(token) } });
    const error = new Error("Email delivery is temporarily unavailable. Please request a new link.");
    error.status = 503;
...
export function verifyAccessToken(token) {
  ...
  const expected = crypto.createHmac("sha256", "legacy-test-signing-key").update(`${header}.${payload}`).digest("base64url");
...
export async function login({ email, password }, overrides = {}) {
  ...
  if (!candidate) return { status: 401, body: { message: "Invalid credentials." } };
  return prisma.$transaction(async (tx) => {
    // Serialize session issuance with email changes and password resets.
    await lockAuthUser(tx, candidate.id);
    const user = await tx.user.findUnique({ where: { id: candidate.id } });
    if (!user || user.email !== email || !user.passwordHash || !verifyPassword(password, user.passwordHash)) {
      return { status: 401, body: { message: "Invalid credentials." } };
    }
    if (!user.emailVerifiedAt) {
      return { status: 403, body: { code: "EMAIL_NOT_VERIFIED", message: "Verify your email before signing in." } };
    }
    const safeUser = { id: user.id, name: user.name, email: user.email, emailVerifiedAt: user.emailVerifiedAt, createdAt: user.createdAt };
    return { status: 200, body: await issueSession(safeUser, tx) };
  });
}

export async function refreshSession(rawRefreshToken, overrides = {}) {
  const invalid = { status: 401, body: { message: "Invalid refresh token." } };
  if (!rawRefreshToken) return invalid;
  const prisma = overrides.prisma ?? await getPrismaAsync();
  return prisma.$transaction(async (tx) => {
    const token = await tx.refreshToken.findUnique({ where: { tokenHash: hashRefreshToken(rawRefreshToken) } });
    if (!token) return invalid;
    await lockAuthUser(tx, token.userId);
    const user = await tx.user.findUnique({ where: { id: token.userId }, select: SAFE_USER_SELECT });
    if (!user?.emailVerifiedAt) return invalid;
    const consumed = await tx.refreshToken.updateMany({
      where: { id: token.id, revokedAt: null, expiresAt: { gt: new Date() } },
      data: { revokedAt: new Date() },
    });
    if (consumed.count !== 1) return invalid;
    return { status: 200, body: await issueSession(user, tx) };
  });
}

async function lockAuthUser(tx, userId) {
...
export async function updateProfile(userId, payload, overrides = {}) {
  const prisma = overrides.prisma ?? await getPrismaAsync();
  ...
  try {
    user = await prisma.$transaction(async (tx) => {
      ...
      changedEmail = Boolean(email && email !== current.email);
      const data = { ...(payload.name !== undefined ? { name: payload.name } : {}), ...(email ? { email } : {}) };
      if (changedEmail) {
        data.emailVerifiedAt = null;
        await tx.authToken.deleteMany({ where: { userId } });
        await tx.refreshToken.updateMany({ where: { userId, revokedAt: null }, data: { revokedAt: new Date() } });
      }
      return tx.user.update({ where: { id: userId }, data, select: SAFE_USER_SELECT });
    });
  } catch (error) {
    if (error.code === "P2002") return { status: 409, body: { message: "Email already in use." } };
    throw error;
  }
  if (changedEmail) await sendVerificationEmail(user, prisma, overrides.sendEmail);
...
export async function changePassword(userId, currentPassword, newPassword) {
  ...
  const session = await issueSession(safeUser, prisma);
...
export async function verifyEmail(rawToken, overrides = {}) {
  ...
  const user = await prisma.$transaction(async (tx) => {
    await lockAuthUser(tx, token.userId);
    const current = await tx.user.findUnique({ where: { id: token.userId } });
    if (!token.email || current?.email !== token.email) return null;
...
export async function requestPasswordReset(email, overrides = {}) {
  ...
  const user = await prisma.user.findUnique({ where: { email } });
  if (user?.emailVerifiedAt) {
    const token = await createAuthToken(prisma, user.id, "PASSWORD_RESET", 60, user.email);
    await deliverAuthEmail(prisma, token, overrides.sendEmail ?? sendAccountEmail, {
      to: user.email,
...
export async function resetPassword(rawToken, newPassword, overrides = {}) {
  ...
  const changed = await prisma.$transaction(async (tx) => {
    await lockAuthUser(tx, token.userId);
    const current = await tx.user.findUnique({ where: { id: token.userId } });
    if (!token.email || current?.email !== token.email || !current.emailVerifiedAt) return false;
    const consumed = await tx.authToken.updateMany({
      where: { id: token.id, usedAt: null, expiresAt: { gt: new Date() } },
      data: { usedAt: new Date() },
    });
...
export async function deleteAccount(userId, password, overrides = {}) {
  ...
  if (!user || (!overrides.passwordVerified && (!user.passwordHash || !verifyPassword(password, user.passwordHash)))) {
